...
import { Eye, ArrowUpDown, ChevronDown, Edit2 } from "lucide-react";
...
export default function OrderManagement({ orders }: { orders: TOrder[] }) {
  ...
  const columns: ColumnDef<TOrder>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <Badge
          variant={
            row.original.prescription ? "default" : "secondary"
          }
        >
          {row.original.prescription ? "Required" : "Not Required"}
        </Badge>
      ),
    },
    {
        ...
        return (
          ...
          <DropdownMenuContent>
            {Object.values(STATUS_OPTIONS).map((status) => (
              <DropdownMenuItem
                key={status}
                onClick={() => handleStatusChange(order._id as string, status)}
              >
                {status}
              </DropdownMenuItem>
            ))}
